...
if [ "$cygwin" = "false" -a "$msys" = "false" ] && ! os400; then
    ...
    if [ $? -eq 0 ] ; then
        ...
        warn "Could not query hard limit for file descriptors: $MAX_FD_LIMIT"
    fi
fi
...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    ROOTDIRSRAW=`find -L / -maxdepth 3 -type d -name build 2>/dev/null`
    SEP=""
